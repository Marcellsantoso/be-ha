...
class InputGallery extends InputFileMultiple {
	...
	public function p ()
	{
		...
		foreach ($exp as $fil) {
			...
			$text .= "<div id='file_" . $exp2[0] .
				"_{$t}' class='mlt_item_gal'><div id='gal_$fil3' class='foto100gal'><span class='helper'></span><img ondblclick='makeasmainpic_" .
				$t . "(\"$fil\");' src='" . _SPPATH . $if->upload_url . $if->file_filename . "'></div>" .
				$if->file_url . " <i onclick=\"deleteFromList_{$t}('" . $if->file_filename .
		...
		<script>
			...
			function uploadFiles_<?=$t;?>(event) {
				...
				$.ajax({
					url: '<?=_SPPATH;?>UploaderMultiples/uploadfiles_gallery?t=<?=$t;?>&files=1',
					type: 'POST',
					data: data,
					cache: false,
					dataType: 'json',
