...
class InputFileModel extends Model {
	...
	public $upload_thumb      = "plugins/jQuery_File_Upload/server/php/inputfiles/";

	function __construct ()
	{

		$this->upload_location = _PHOTOPATH . "";
		$this->upload_url = _PHOTOURL . "";
		$this->upload_thumb = _PHOTOURL. "thumbnail/";
	...
	public function printLink ()
	{
		...
		if (in_array($this->file_ext, $inp->arrImgExt)) {
			return "<div id='file_" . $this->file_id .
			"' $onclick class='pl_file_item'><div class='fotoIF'><img onload='OnImageLoad(event, 30);' src='" . $path .
			$fil . "'></div><div class='if_text'>" . $this->file_url . "</div><div class='clearfix'></div></div>";
		}
		if ($this->file_ext == "pdf") {
			...
			if (file_exists($thumb)) {
				...
				return "<div id='file_" . $this->file_id .
				"' $onclick class='pl_file_item'><div class='fotoIF'><img onload='OnImageLoad(event, 30);' src='" .
				$thumburl . "'></div><div class='if_text'>" . $this->file_url .
